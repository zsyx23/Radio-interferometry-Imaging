...
Maxiter= 100 ;soft_threshold_values=1e-6; forward_descent_step_size=0.49;
lambda_IUWT= 1e-5 ;  gamma_IUWT = 0.1 ;  iter_max_IUWT  =  100 ;
gamma_WTF= 0.5 ;  iter_max_WTF= 100 ;
...
SNR_sum_ave_PPD=zeros(1,length(per));
SNR_sum_ave_ADMM=zeros(1,length(per));
SNR_sum_ave_IUWT=zeros(1,length(per));
SNR_sum_ave_WTF=zeros(1,length(per)); 
SNRlog_sum_ave_PPD=zeros(1,length(per));
SNRlog_sum_ave_ADMM=zeros(1,length(per));
SNRlog_sum_ave_IUWT=zeros(1,length(per));
SNRlog_sum_ave_WTF=zeros(1,length(per)); 
TIME_sum_ave_PD=zeros(1,length(per));
TIME_sum_ave_PPD=zeros(1,length(per));
TIME_sum_ave_ADMM=zeros(1,length(per));
TIME_sum_ave_IUWT=zeros(1,length(per));
TIME_sum_ave_WTF=zeros(1,length(per));
DR_sum_ave_PPD=zeros(1,length(per));
DR_sum_ave_ADMM=zeros(1,length(per));
DR_sum_ave_IUWT=zeros(1,length(per));
DR_sum_ave_WTF=zeros(1,length(per));
...
param_pdfb.gamma = soft_threshold_values ; % convergence parameter L1 (soft th parameter)
% param_pdfb_precond.tau = 0.49; % forward descent step size
param_pdfb.tau = forward_descent_step_size ; % forward descent step size
...
TIME_PPD=tend;
DR_PPD = max(soln_PPD(:))/(norm(residualn_PPD(:))/sqrt(N));

soln_SNR=soln_PPD;
standard_deviation=std2(soln_SNR);
noise_level_estimate=standard_deviation./evl;
one_N=ones(size(soln_SNR));
SNRlog_PPD=20*log10(norm(log10(soln_SNR(:)./noise_level_estimate+one_N(:)))./norm(log10(soln_SNR(:)./noise_level_estimate+one_N(:))-log10(im(:)./noise_level_estimate+one_N(:))));
...
TIME_ADMM =tend;
DR_ADMM = max(soln_ADMM(:))/(norm(residualn_ADMM(:))/sqrt(N));

soln_SNR=soln_ADMM;
standard_deviation=std2(soln_SNR);
noise_level_estimate=standard_deviation./evl;
one_N=ones(size(soln_SNR));
SNRlog_ADMM=20*log10(norm(log10(soln_SNR(:)./noise_level_estimate+one_N(:)))./norm(log10(soln_SNR(:)./noise_level_estimate+one_N(:))-log10(im(:)./noise_level_estimate+one_N(:))));
...
TIME_IUWT =tend;
DR_IUWT = max(soln_IUWT(:))/(norm(residualn_IUWT(:))/sqrt(N));

soln_SNR=soln_IUWT;
standard_deviation=std2(soln_SNR);
noise_level_estimate=standard_deviation./evl;
one_N=ones(size(soln_SNR));
SNRlog_IUWT=20*log10(norm(log10(soln_SNR(:)./noise_level_estimate+one_N(:)))./norm(log10(soln_SNR(:)./noise_level_estimate+one_N(:))-log10(im(:)./noise_level_estimate+one_N(:))));
...
TIME_WTF =tend;
DR_WTF = max(soln_WTF(:))/(norm(residualn_WTF(:))/sqrt(N));

soln_SNR=soln_WTF;
standard_deviation=std2(soln_SNR);
noise_level_estimate=standard_deviation./evl;
one_N=ones(size(soln_SNR));
SNRlog_WTF=20*log10(norm(log10(soln_SNR(:)./noise_level_estimate+one_N(:)))./norm(log10(soln_SNR(:)./noise_level_estimate+one_N(:))-log10(im(:)./noise_level_estimate+one_N(:))));
...
SNR_sum_PPD=[SNR_sum_PPD SNR_PPD];
SNR_sum_ADMM=[SNR_sum_ADMM SNR_ADMM];
SNR_sum_IUWT=[SNR_sum_IUWT SNR_IUWT];
SNR_sum_WTF=[SNR_sum_WTF SNR_WTF];

SNRlog_sum_PPD=[SNRlog_sum_PPD SNRlog_PPD];
SNRlog_sum_ADMM=[SNRlog_sum_ADMM SNRlog_ADMM];
SNRlog_sum_IUWT=[SNRlog_sum_IUWT SNRlog_IUWT];
SNRlog_sum_WTF=[SNRlog_sum_WTF SNRlog_WTF];

TIME_sum_PD=[TIME_sum_PD TIME_PD];
TIME_sum_PPD=[TIME_sum_PPD TIME_PPD];
TIME_sum_ADMM=[TIME_sum_ADMM TIME_ADMM];
TIME_sum_IUWT=[TIME_sum_IUWT TIME_IUWT];
TIME_sum_WTF=[TIME_sum_WTF TIME_WTF];

DR_sum_PPD=[DR_sum_PPD DR_PPD];
DR_sum_ADMM=[DR_sum_ADMM DR_ADMM];
DR_sum_IUWT=[DR_sum_IUWT DR_IUWT];
DR_sum_WTF=[DR_sum_WTF DR_WTF];

close all;

end

SNR_sum_ave_PD=SNR_sum_ave_PD +SNR_sum_PD ;
SNR_sum_ave_PPD=SNR_sum_ave_PPD +SNR_sum_PPD ;
SNR_sum_ave_ADMM=SNR_sum_ave_ADMM +SNR_sum_ADMM ;
SNR_sum_ave_IUWT=SNR_sum_ave_IUWT +SNR_sum_IUWT ;
SNR_sum_ave_WTF=SNR_sum_ave_WTF +SNR_sum_WTF ;

SNRlog_sum_ave_PPD=SNRlog_sum_ave_PPD +SNRlog_sum_PPD ;
SNRlog_sum_ave_ADMM=SNRlog_sum_ave_ADMM +SNRlog_sum_ADMM ;
SNRlog_sum_ave_IUWT=SNRlog_sum_ave_IUWT +SNRlog_sum_IUWT ;
SNRlog_sum_ave_WTF=SNRlog_sum_ave_WTF +SNRlog_sum_WTF ;

TIME_sum_ave_PD=TIME_sum_ave_PD +TIME_sum_PD ;
TIME_sum_ave_PPD=TIME_sum_ave_PPD +TIME_sum_PPD ;
TIME_sum_ave_ADMM=TIME_sum_ave_ADMM +TIME_sum_ADMM ;
TIME_sum_ave_IUWT=TIME_sum_ave_IUWT +TIME_sum_IUWT ;
TIME_sum_ave_WTF=TIME_sum_ave_WTF +TIME_sum_WTF ;

DR_sum_ave_PPD=DR_sum_ave_PPD +DR_sum_PPD ;
DR_sum_ave_ADMM=DR_sum_ave_ADMM +DR_sum_ADMM ;
DR_sum_ave_IUWT=DR_sum_ave_IUWT +DR_sum_IUWT ;
DR_sum_ave_WTF=DR_sum_ave_WTF +DR_sum_WTF ;

end

SNR_sum_ave_PD=SNR_sum_ave_PD / average_times ;
SNR_sum_ave_PPD=SNR_sum_ave_PPD / average_times ;
SNR_sum_ave_ADMM=SNR_sum_ave_ADMM / average_times ;
SNR_sum_ave_IUWT=SNR_sum_ave_IUWT / average_times ;
SNR_sum_ave_WTF=SNR_sum_ave_WTF / average_times ;

SNRlog_sum_ave_PPD=SNRlog_sum_ave_PPD / average_times ;
SNRlog_sum_ave_ADMM=SNRlog_sum_ave_ADMM / average_times ;
SNRlog_sum_ave_IUWT=SNRlog_sum_ave_IUWT / average_times ;
SNRlog_sum_ave_WTF=SNRlog_sum_ave_WTF / average_times ;

TIME_sum_ave_PD=TIME_sum_ave_PD / average_times ;
TIME_sum_ave_PPD=TIME_sum_ave_PPD / average_times ;
TIME_sum_ave_ADMM=TIME_sum_ave_ADMM / average_times ;
TIME_sum_ave_IUWT=TIME_sum_ave_IUWT / average_times ;
TIME_sum_ave_WTF=TIME_sum_ave_WTF / average_times ;

DR_sum_ave_PPD=DR_sum_ave_PPD / average_times ;
DR_sum_ave_ADMM=DR_sum_ave_ADMM / average_times ;
DR_sum_ave_IUWT=DR_sum_ave_IUWT / average_times ;
DR_sum_ave_WTF=DR_sum_ave_WTF / average_times ;
...
figure;
% plot(xx,SNR_sum_ave_PD,'m-s');hold on;
plot(xx,SNR_sum_ave_PPD,'m-v');hold on;
plot(xx,SNR_sum_ave_ADMM,'k-d');hold on;
plot(xx,SNR_sum_ave_IUWT,'b-s');hold on;
plot(xx,SNR_sum_ave_WTF,'r-o');hold on;
% legend('PD','PPD','ADMM','IUWT','WTF','FontName','Times New Roman','FontSize',12);
% xlim([0 0.5]);
xlabel('Coverage percentage','FontName','Times New Roman','FontSize',16);
ylabel('SNR (dB)','FontName','Times New Roman','FontSize',16);

figure;
% plot(xx,TIME_sum_ave_PD,'m-s');hold on;
plot(xx,TIME_sum_ave_PPD,'m-v');hold on;
plot(xx,TIME_sum_ave_ADMM,'k-d');hold on;
plot(xx,TIME_sum_ave_IUWT,'b-s');hold on;
plot(xx,TIME_sum_ave_WTF,'r-o');hold on;
% legend('PD','PPD','ADMM','IUWT','WTF','FontName','Times New Roman','FontSize',12);
% xlim([0 0.5]);
xlabel('Coverage percentage','FontName','Times New Roman','FontSize',16);
ylabel('Time (s)','FontName','Times New Roman','FontSize',16);


figure;
plot(xx,SNRlog_sum_ave_PPD,'m-v');hold on;
plot(xx,SNRlog_sum_ave_ADMM,'k-d');hold on;
plot(xx,SNRlog_sum_ave_IUWT,'b-s');hold on;
plot(xx,SNRlog_sum_ave_WTF,'r-o');hold on;
% legend('PD','PPD','ADMM','IUWT','WTF','FontName','Times New Roman','FontSize',12);
% xlim([0 0.5]);
xlabel('Coverage percentage','FontName','Times New Roman','FontSize',16);
ylabel('SNRlog (dB)','FontName','Times New Roman','FontSize',16);
