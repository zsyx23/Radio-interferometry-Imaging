...
TIME_sum_Hyper=[];
DR_sum_Hyper=[];
SNR_sum_ave_Hyper=zeros(1,length(per));
SNRlog_sum_ave_Hyper=zeros(1,length(per));
TIME_sum_ave_Hyper=zeros(1,length(per));
DR_sum_ave_Hyper=zeros(1,length(per));
...
% param_HSI.elipse_proj_max_iter = 40;
...
% 结果
figure; imagesc(real(log10((xsol(:,:,1)))));
set(gca,'YDir','normal');
axis off;
colorbar, axis image; caxis([-2,0]);
colormap(cubehelix);
set(gca,'position',[-0.05,0.02,0.98,0.934])
set(gcf,'unit','normalized','position',[0.2,0.2,0.26,0.4]);
...
figure; imagesc(real(((res(:,:,1)))));
set(gca,'YDir','normal');
axis off;
colorbar, axis image;% caxis([-0.001,0.001]);
colormap(cubehelix);
set(gca,'position',[-0.05,0.02,0.98,0.934])
set(gcf,'unit','normalized','position',[0.2,0.2,0.26,0.4]);
...
% 误差
error=x0-xsol;
figure; imagesc(real(((error(:,:,1)))));
set(gca,'YDir','normal');
axis off;
colorbar, axis image; caxis([-0.05,0.05]);
colormap(cubehelix);
set(gca,'position',[-0.05,0.02,0.98,0.934])
set(gcf,'unit','normalized','position',[0.2,0.2,0.26,0.4]);
...
soln_=xsol(:,:,1);
DR_Hyper = max(soln_(:))/(norm(res(:))/sqrt(N));

soln_SNR=soln_;
standard_deviation=std2(soln_SNR);
noise_level_estimate=standard_deviation./Anorm;
one_N=ones(size(soln_SNR));
SNRlog_Hyper=20*log10(norm(log10(soln_SNR(:)./noise_level_estimate+one_N(:)))./norm(log10(soln_SNR(:)./noise_level_estimate+one_N(:))-log10(x0(:)./noise_level_estimate+one_N(:))));
...
SNR_sum_Hyper=[SNR_sum_Hyper SNR_Hyper];
SNRlog_sum_Hyper=[SNRlog_sum_Hyper SNRlog_Hyper];
TIME_sum_Hyper=[TIME_sum_Hyper TIME_Hyper];
DR_sum_Hyper=[DR_sum_Hyper DR_Hyper];
close all;
end
SNR_sum_ave_Hyper=SNR_sum_ave_Hyper +SNR_sum_Hyper ;
SNRlog_sum_ave_Hyper=SNRlog_sum_ave_Hyper +SNRlog_sum_Hyper ;
TIME_sum_ave_Hyper=TIME_sum_ave_Hyper +TIME_sum_Hyper ;
DR_sum_ave_Hyper=DR_sum_ave_Hyper +DR_sum_Hyper ;
end

SNR_sum_ave_Hyper=SNR_sum_ave_Hyper / average_times ;
SNRlog_sum_ave_Hyper=SNRlog_sum_ave_Hyper / average_times ;
TIME_sum_ave_Hyper=TIME_sum_ave_Hyper / average_times ;
DR_sum_ave_Hyper=DR_sum_ave_Hyper / average_times ;

xx = per;

figure;
plot(xx,SNR_sum_ave_Hyper,'m-v');hold on;
xlabel('Coverage percentage','FontName','Times New Roman','FontSize',16);
ylabel('SNR (dB)','FontName','Times New Roman','FontSize',16);

figure;
plot(xx,TIME_sum_ave_Hyper,'m-v');hold on;
xlabel('Coverage percentage','FontName','Times New Roman','FontSize',16);
ylabel('TIME (s)','FontName','Times New Roman','FontSize',16);

figure;
plot(xx,DR_sum_ave_Hyper,'m-v');hold on;
xlabel('Coverage percentage','FontName','Times New Roman','FontSize',16);
ylabel('DR','FontName','Times New Roman','FontSize',16);

figure;
plot(xx,SNRlog_sum_ave_Hyper,'m-v');hold on;
xlabel('Coverage percentage','FontName','Times New Roman','FontSize',16);
ylabel('SNRlog','FontName','Times New Roman','FontSize',16);
